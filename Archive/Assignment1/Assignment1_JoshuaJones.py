...
def quicksort(nums:list, p:int, r:int, rand_pivot:bool) -> None:
    if p < r:
        if rand_pivot:
...
if __name__ == "__main__":
    ...
    test_vectors = 50
    cases = generate_inputs(cases=test_vectors, max_n=10000)

    # Run all generates test vectors
    det_qs_sorted_t   = []
    det_qs_unsorted_t = []
    random_qs_sorted_t   = []
    ...
    for n,(sorted,unsorted) in cases.items():
        try:
            ...
            random_input = unsorted.copy()
            det_qs_sorted_t.append(  algo_timer(det_qs, sorted_input,  True, n))
            det_qs_unsorted_t.append(algo_timer(det_qs, random_input, False, n))
            assert (sorted == sorted_input), f'\n\ndet_qs: Sorted does not match sorted_input\n{sorted}\n{sorted_input}\n' 
            assert (sorted == random_input), f'\n\ndet_qs: Sorted does not match random_input\n{sorted}\n{random_input}\n' 

            # Random Quick Sort
            sorted_input = sorted.copy()
            random_input = unsorted.copy()
            random_qs_sorted_t.append(  algo_timer(random_qs, sorted_input,  True, n))
            random_qs_unsorted_t.append(algo_timer(random_qs, random_input, False, n))
            assert (sorted == sorted_input), f'\n\nrandom_qs: Sorted does not match sorted_input\n{sorted}\n{sorted_input}\n' 
            assert (sorted == random_input), f'\n\nrandom_qs: Sorted does not match random_input\n{sorted}\n{random_input}\n' 

            # Merge Sort
            sorted_input = sorted.copy()
            random_input = unsorted.copy()
            merge_sort_sorted_t.append(  algo_timer(merge_sort, sorted_input,  True, n))
            merge_sort_unsorted_t.append(algo_timer(merge_sort, random_input, False, n))
            assert (sorted == sorted_input), f'\n\nmerge_sort: Sorted does not match sorted_input\n{sorted}\n{sorted_input}\n' 
            assert (sorted == random_input), f'\n\nmerge_sort: Sorted does not match random_input\n{sorted}\n{random_input}\n' 
        except:
            e_cnt += 1
            pass
    ...
    print(f'\nCode Coverage: {((test_vectors - e_cnt) / test_vectors):.0%}\nTotal Runtime: {t_total_duration:0.03} s\n')
